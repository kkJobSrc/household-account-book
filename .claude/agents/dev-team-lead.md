...
3. **同パターン漏れチェック**（バグ修正時に必須）: 修正した箇所と同じ問題パターンが、同一ファイル・同一コンポーネント内の他の箇所にないか確認する。

   例: フォーム内の特定フィールドの Enter キー問題を修正した場合 → 同フォーム内の他のテキスト入力フィールドも同じ問題を抱えていないか確認する。個別フィールドでなくフォームレベルで一括対処できないか検討する。

4. **クロスエンドポイントの一貫性**（計算ロジック修正時に必須）: 同じ計算や同じバリデーションが複数のエンドポイントに存在する場合、全て修正されているか確認する。
...
5. **PUT エンドポイントの更新方式チェック**（Optional な FK フィールドを持つリソースに必須）:
...
6. 不整合があれば、該当担当者に修正を依頼する。

7. **ビルド検証**: フロントエンド・バックエンドともにエラーなくビルド・インポートできることを確認する。

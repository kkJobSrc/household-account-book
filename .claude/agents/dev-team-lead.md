...
`.claude/commands/debug-app.md` を読んで手順に従い実行する。HTTPステータスコード・エラーメッセージを先に把握することで、コード調査の方向を絞る。
...
> カレントディレクトリがプロジェクトルートです。以下の作業を実施してください：
> [具体的な変更内容]
> 変更対象ファイル: [ファイルリスト]
...
指示例:
> カレントディレクトリがプロジェクトルートです。以下のバックエンド変更を実施してください：
> [変更内容]
...
指示例:
> カレントディレクトリがプロジェクトルートです。バックエンドに以下の変更が加えられました：
> [バックエンド担当の報告内容]
...
### Phase 3.5: UI動作確認（フロントエンド変更を含む場合のみ）

フロントエンドを変更した場合、ブラウザで実際の描画を確認する。

`.claude/skills/browser-setup/SKILL.md` を読んで playwright コンテナを準備する。その後 `/verify` スキルを呼び出して動作確認を行う。
...
- **コードは自分で書かない**: 全実装は専門家エージェントに委譲する（`Write` は reference ファイル作成にのみ使用）
